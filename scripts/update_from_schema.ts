...
function updateEphemeralMapping(schema: JSONSchema) {
    ...
    let mappingFilePath = '../deploy/ephemeral.yaml'
    let mappingFileContent = fs.readFileSync(mappingFilePath, 'utf8');
    let loaded: any = YAML.load(mappingFileContent);
    let objects: any[] = [{}];
    ...
    if(_.has(xjoinConfigMap, "data")) {
        ...
        xjoinConfigMap["data"]["elasticsearch.index.template"] = ">-\n" + JSON.stringify(new_mapping["mappings"]["system_profile_facts"]["properties"], null, 2);
    }
    ...
    // // fs.writeFileSync(mappingFile  Path, JSON.stringify(template, null, 2));
    fs.writeFileSync(mappingFilePath, YAML.dump(loaded));
}
